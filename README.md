...
- **Public status pages** — read-only views per-monitor with the recent_check ring buffer rendered as a chart.
- **Auth** — gated by Cloudflare Access; no app-side login UI. Allowlist by email.
...
| Worker | Config | Role |
|---|---|---|
| `uptime-scarecrow-app` | `wrangler.jsonc` | Astro server output — dashboard, public status pages, JSON API, heartbeat ingest. |
...
1. **Settings → Custom Pages → Identity** — set up at least one identity provider (Google, GitHub, one-time PIN, etc.).
2. **Access → Applications → Add an application → Self-hosted.**
3. Application domain: the hostname you'll deploy the app to (e.g. `uptime.example.com`).
...
- **Auth.** All non-public routes require a valid Cloudflare Access JWT (verified via JWKS, RS256). Email allowlist enforced after JWT verification. `/h/<token>`, `/status/*`, `/about`, `/_image`, `/favicon`, `/robots.txt` are public by design.

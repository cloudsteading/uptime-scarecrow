...
const fail = (ctx: APIContext, id: number, msg: string) =>
  flashRedirect(ctx, `/admin/monitor/${id}/edit`, msg, 'err');
...
export const POST: APIRoute = async (ctx) => {
  ...
  if (!name) return fail(ctx, id, 'Name is required');
  ...
  if (type === 'http') {
    ...
    try { new URL(url); } catch { return fail(ctx, id, 'Invalid URL'); }
    if (!['GET', 'HEAD', 'POST'].includes(method)) return fail(ctx, id, 'Bad method');
    ...
    if (kind === 'cron') {
      ...
      if (!cron_expression) return fail(ctx, id, 'Cron expression required');
      const v = validateCron(cron_expression);
      if (!v.ok) return fail(ctx, id, `Invalid cron: ${v.error}`);
    } else {
      interval_seconds = Number(form.get('interval_seconds') ?? 300);
      if (![60, 300, 900, 3600].includes(interval_seconds)) return fail(ctx, id, 'Bad interval');
    }
    ...
    return flashRedirect(ctx, `/admin/monitor/${id}`, 'Monitor saved');
  }
  ...
  if (type === 'heartbeat') {
    ...
    if (kind === 'cron') {
      ...
      if (interval_seconds < 60 || interval_seconds > 30 * 86400) return fail(ctx, id, 'Interval out of range');
    }
  ...
  return fail(ctx, id, 'Unknown monitor type');
};
